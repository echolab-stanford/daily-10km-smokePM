...
dates_oddly_empty = readRDS(paste0(path_dropbox, "hms_hysplit/hysplit_dates_oddly_empty.rds"))
...
traj_points_files = list.files(paste0(path_seagate, "HYSPLIT/6-day trajectories/trajectory points/"), pattern = "^trajectory_points.*\\.rds$")
...
traj_points_files = paste0(path_seagate, "HYSPLIT/6-day trajectories/trajectory points/", traj_points_files)
...
stopImplicitCluster()
saveRDS(heights, paste0(path_seagate, "HYSPLIT/miscellaneous/grid_trajectories/6-day/heights.rds"))
heights = readRDS(paste0(path_seagate, "HYSPLIT/miscellaneous/grid_trajectories/6-day/heights.rds"))
...
project_grid = readRDS(paste0(path_seagate, "HYSPLIT/10km_grid_HYSPLIT_crs.rds"))
...
for (m in 1:length(year_months)) {
  ...
  for (d in 1:length(dates_m)) {
    ...
    } else if (dates_m[d] %in% na_dates_str) {
      traj_points_grid_d = traj_points_grid_d %>% mutate(across(-c(id_grid, date), ~NA))
    }
  ...
  traj_points_grid_file = paste0(path_dropbox, "hms_hysplit/10km_grid_2006-2020/grid_trajectory_points_", year_month, ".rds")
