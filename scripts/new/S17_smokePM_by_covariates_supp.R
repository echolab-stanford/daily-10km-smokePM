...
acs <- list.files("~/BurkeLab Dropbox/Data/US Census Bureau/American Community Survey/5-Year",
                    pattern = "2009|2019",
                    full.names = T) %>% 
...
gdp <- read.csv("./scratch/CAGDP9/CAGDP9__ALL_AREAS_2001_2020.csv") %>% 
  filter(LineCode == 1) %>% 
...
smokePM_pred <- readRDS(paste0(output_path, "/smokePM_predictions_20060101_20201231.rds"))
...
# "GDP~per~capita~($1,000, 2012)"
# "paste(\"Income per capita ($1,000)\")"
g_spline_hist <- spline_fits %>% 
  ...
  # mutate(covar = ifelse(name == "Population~density~(population/km^2)", sinh(covar), covar)) %>%
...
g_spline_hist %>% 
  ...
  gtable_add_grob(g_maps[[2]] %>% ggplot_build() %>% ggplot_gtable(),
                  t = 1, b = 2, l = 9, r = 9) %>%
  ggsave("./figures/supplement/figureSX_maps_avgs5yr.png", ., 
         width = 6, height = 4.5)
