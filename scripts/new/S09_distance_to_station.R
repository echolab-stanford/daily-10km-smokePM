...
library(sf)
grid <- read_sf(paste0(data_path, "/1_grids/10km_grid"))
...
# smoke PM data 
epa_stations <- readRDS(paste0(data_path, "/3_intermediate/station_smokePM.rds")) %>% 
  pull(id) %>% 
  unique
...
pdf("./figures/supplement/SX_grid_to_station.pdf", 
    width = 10, height = 5)
