...
get_plumes = function(dt) {
  ...
  dt = dt[, total := ifelse(date!=20150125, rowSums(.SD), total), .SDcols=2:4]
  return(dt)
}
...
# details about dates
dates_not_online = readRDS(paste0(db_path, "/Data/smoke/smoke_dates_not_online.rds"))
dates_empty_data = readRDS(paste0(db_path, "/Data/smoke/smoke_dates_empty_data.rds"))
dates_invalid_geometry = readRDS(paste0(db_path, "/Data/smoke/smoke_dates_invalid_geometry.rds"))

smoke = combined %>% 
  select(id_grid = id, date, smoke_day, total, light, medium, dense) %>% 
