...
smokePM_preds %>% 
  ...
  ggsave(filename = "./figures/figure2/figure2_pred_obs.png", 
         ., width = 7, height = 5)

# panel B, variable importance 
list.files("./output/smokePM_model", pattern = "var_importance", 
           full.names = T) %>% 
  grep("fold99", ., value = T) %>% 
  readRDS() %>% 
...
station_performance %>% 
  ...
  {{ggplot(data = .) +
      ...
      scale_colour_gradientn(colors = brewer.pal(11, "RdYlBu"),
                             ...
                             guide = "none") +
      theme_void()} %>%
      ggsave(filename = "./figures/figure2/figure2_spatial_r2.png",
             plot = .,
             width =7, height = 5)
    {ggplot(data = mutate(., hist_col = cut(r2, breaks = seq(0, 1, length.out = 12))), 
        ...
        xlab(expression(R^2)) + 
        theme(text = element_text(size = 18))} %>% 
      ggsave(filename = "./figures/figure2/figure2_station_r2_hist.png", 
             plot = ., 
...
epa_ll %>% 
  ...
  {ggplot(data = .) + 
      ...
      scale_color_manual(name = "landcover", 
                         values = c("#6e948c", "black", "#c38f16")) + # c("#2c6b67", "black", "#99610a")
      theme_void()} %>% 
  ggsave("./figures/supplement/SX_station_modis_landcover.png",. ,
         width = 6, height = 4)
...
mod_data %>% st_drop_geometry() %>% 
  ...
  ggsave("./figures/figure2/predictors_station_performance.png", ., width = 7, height = 5)