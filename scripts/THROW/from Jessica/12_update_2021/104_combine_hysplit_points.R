# Combine HYSPLIT points post-2020
...
# Get dates starting from start_date and ending at end_date
dates = as.character(parse_number(gsub("\\.", "", files)))
dates_run = unique(dates[which(dates >= start_date & dates <= end_date)])
...
# Save
saveRDS(dat_hms_hysplit, paste0(path_dropbox, sprintf("hms_hysplit/hms_hysplit_points_%s-%s.rds", start_date, end_date)))
