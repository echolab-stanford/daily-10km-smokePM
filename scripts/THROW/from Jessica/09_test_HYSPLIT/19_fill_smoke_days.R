...
#-------------------------------------------------------------------------------
# Set thresholds
min_num_traj_points = 50 # minimum traj point count if HYSPLIT points
...
filled_smoke_days = vector("list", length(year_months))
...
for (m in 1:length(year_months)) {
  ...
  traj = paste0(path_dropbox, "hms_hysplit/10km_grid_2006-2020/grid_trajectory_points_", year_month, ".rds")
  traj = readRDS(traj) %>% 
  ...
  aod_missing = paste0(path_project, "data/2_from_EE/maiac_AODmissings/", aod_missing)
  ...
  smoke_days = paste0(path_project, "data/smoke_days/grid_smoke_day_", year_month, ".rds")
  ...
  if (has_missing_date) {
    if (min(dates_m) != start_date) {
      prev_smoke_days = paste0(path_project, "data/smoke_days/grid_smoke_day_", prev_ym_str, ".rds")
      prev_smoke_days = readRDS(prev_smoke_days) %>% 
        mutate(date = format(date, "%Y%m%d")) %>% 
        select(grid_id_10km = id_grid, date, smoke_day, note_smoke_date_not_online)
      smoke_days = bind_rows(smoke_days, prev_smoke_days)
      
      prev_traj = paste0(path_dropbox, "hms_hysplit/10km_grid_2006-2020/grid_trajectory_points_", prev_ym_str, ".rds")
      ...
      prev_aod_missing = paste0(path_project, "data/2_from_EE/maiac_AODmissings/", prev_aod_missing)
    ...
    if (max(dates_m) != end_date) {
      foll_smoke_days = paste0(path_project, "data/smoke_days/grid_smoke_day_", foll_ym_str, ".rds")
      foll_smoke_days = readRDS(foll_smoke_days) %>% 
        mutate(date = format(date, "%Y%m%d")) %>% 
        select(grid_id_10km = id_grid, date, smoke_day, note_smoke_date_not_online)
      smoke_days = bind_rows(smoke_days, foll_smoke_days)
      
      foll_traj = paste0(path_dropbox, "hms_hysplit/10km_grid_2006-2020/grid_trajectory_points_", foll_ym_str, ".rds")
      ...
      foll_aod_missing = paste0(path_project, "data/2_from_EE/maiac_AODmissings/", foll_aod_missing)
  ...
  filled_smoke_days[[m]] = smoke_days
}
print_time(start_time)

filled_smoke_days = bind_rows(filled_smoke_days) %>% arrange(date, grid_id_10km)

saveRDS(filled_smoke_days, paste0(path_project, "data/3_intermediate/all_smoke_days_incl_cloudy.rds"))

#-------------------------------------------------------------------------------
# How many additional grid cell-smoke days do we get from cloudy trajectory-based
# smoke days undetected by plumes?
filled_smoke_days_wo_cloudy = readRDS(paste0(path_project, "data/3_intermediate/all_smoke_days.rds"))
