    ...
    for (cut in co) {
      ...
      for (f in 1:5) {
        ...
        params <- makeParamSet(makeIntegerParam("mtry", 2, nf - 2),
                               ...
                               makeDiscreteParam("sample.fraction", c(0.6, 0.8, 1)),
        ...
        if (spec == "baseline") {
          saveRDS(mdl, paste0(path_project, sprintf("using_ranger/HYSPLIT_model_%s_%s.rds", spec, f)))
          saveRDS(predict(mdl, dat_merged)$predictions, paste0(path_project, sprintf("using_ranger/HYSPLIT_predictions_%s_%s.rds", spec, f)))
        } else {
          saveRDS(mdl, paste0(path_project, sprintf("using_ranger/HYSPLIT_model_%s_%s_%s_%s.rds", spec, inj, cut, f)))
          saveRDS(predict(mdl, dat_merged)$predictions, paste0(path_project, sprintf("using_ranger/HYSPLIT_predictions_%s_%s_%s_%s.rds", spec, inj, cut, f)))
        }
      }
    }
  }
...
m <- 1
for (f in 1:5) {
  mdl <- readRDS(paste0(path_project, sprintf("using_ranger/HYSPLIT_model_baseline_%s.rds", f)))
  mdls[[m]] <- list(model = mdl,
                    spec = "baseline",
                    fold = f)
  dat_preds[, sprintf("pred.baseline.%s", f)] <- readRDS(paste0(path_project, sprintf("using_ranger/HYSPLIT_predictions_baseline_%s.rds", f)))
  m <- m + 1
}
for (spec in specs[-1]) {
  for (inj in gsub("\\+", "-", injection_heights)) {
    for (cut in cutoffs) {
      for (f in 1:5) {
        mdl <- readRDS(paste0(path_project, sprintf("using_ranger/HYSPLIT_model_%s_%s_%s_%s.rds", spec, inj, cut, f)))
        mdls[[m]] <- list(model = mdl,
                          spec = spec,
                          fold = f,
                          injection_heights = gsub("-", "\\+", inj),
                          cutoff = cut)
        dat_preds[, sprintf("pred.%s.%s.%s.%s", spec, f, cut, gsub("-", "_", inj))] <- readRDS(paste0(path_project, sprintf("using_ranger/HYSPLIT_predictions_%s_%s_%s_%s.rds", spec, inj, cut, f)))
        m <- m + 1
      }
    }
  }
...
saveRDS(mdls, paste0(path_project, "using_ranger/HYSPLIT_random_forest_models.rds"))
saveRDS(dat_preds, paste0(path_project, "using_ranger/HYSPLIT_random_forest_predictions.rds"))
