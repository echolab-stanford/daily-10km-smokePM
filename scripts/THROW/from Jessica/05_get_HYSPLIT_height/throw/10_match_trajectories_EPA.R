...
dat_epa <- readRDS(paste0(path_dropbox, "PM25/EPA/epa_station_level_pm25_data.rds")) %>% 
  select(id, lat, lon, date, pm25, county, county_code, cbsa_code, cbsa_name) %>% 
...
# Save spatially matched file
# saveRDS(dat_matched, paste0(path_dropbox, "hms_hysplit/trajectories/trajectories_EPA_CA_2020_locpairs.rds"))
...
nr <- nrow(dat_matched)
# Set number of chunks: 1000 rows per chunk when using 6 cores, decrease if more cores
nc <- ceiling(nr/1000)
...
dat_matched <- foreach(i = 1:nc, .combine = bind_rows, .multicombine = TRUE) %dopar% {
  ...
  for (t in tzs) {
    ...
    date_traj <- loc_pairs[same_tz, "traj_dt"]
    loc_pairs[same_tz, "date_traj"] <- date_traj %>% strftime(format = "%Y-%m-%d", tz = t)
  }
...
# Save matched data
saveRDS(dat_matched, paste0(path_dropbox, "hms_hysplit/trajectories/trajectories_EPA_CA_2020_matched.rds"))
