...
# State
countries <- unique(dat_hms_hysplit$admin)
countries <- countries[!is.na(countries)]
states <- ne_states(country = countries)
states <- SpatialPolygonsDataFrame(SpatialPolygons(states@polygons),
                               states@data %>% select(name))
o <- over(SpatialPoints(hysplit_points), states)
dat_hms_hysplit <- dat_hms_hysplit %>% 
  left_join(bind_cols(hysplit_points, o)) %>% 
  rename(state = name) %>% 
  select(-admin)
...
#-------------------------------------------------------------------------------
# Save data
saveRDS(dat_hms_hysplit, paste0(path_dropbox, "hms_hysplit/hms_hysplit_initialization_distinct_rnaturalearth.rds"))
