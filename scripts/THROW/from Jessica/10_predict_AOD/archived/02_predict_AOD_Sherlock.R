...
# 
# TODO: what to do about NA AOT lags 1 and 2?
#-------------------------------------------------------------------------------
# Set time period
start_date = "20190101"
end_date = "20201231"
...
start_time = get_start_time()
elev <- list.files(paste0(path_data,"2_from_EE/elevation_1km_subgrid"), 
                   full.names = TRUE) %>% 
...
print(paste("NLCD", "--------------------------------------------------"))
start_time = get_start_time()
nlcd <- list.files(paste0(path_data,"2_from_EE/NLCD_1km_subgrid"), full.names = TRUE) %>% 
  map_dfr(function(x) read.csv(x)) %>%  #  
...
nlcd = nlcd %>% select(-`NA`, -total) %>% drop_na()
...
for (m in 1:length(year_months)) {
  ...
  out_m = foreach(d = 1:length(dates_m), .combine = c) %dopar% {
    ...
    print(paste0(ymd_str, "1km predictions saved"))
    ...
    print(paste0(ymd_str, "10km aggregates of predictions saved"))
    return(0)
  }
  out[[m]] = out_m
  print_time(start_time)
