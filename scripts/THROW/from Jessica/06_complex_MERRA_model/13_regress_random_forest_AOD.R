...
for (s in 1:2) {
  ...
  for (f in 1:5) {
    ...
    params <- makeParamSet(makeIntegerParam("mtry", 2, nf - 1),#2, nf),
                           makeIntegerParam("min.node.size", 10, 100),#100, 1000),
    ...
    mdl <- ranger(smokePM ~ .,
                  data = dat_train,
                  min.node.size = 10000,#res$x$min.node.size,
                  mtry = 8,#res$x$mtry,
                  sample.fraction = 0.7,#res$x$sample.fraction,
                  num.trees = 100,#500,
                  importance = "impurity")
    
    # Save model and predictions
    saveRDS(mdl, paste0(path_project, sprintf("AOD_model_spec%s_fold%s.rds", s, f)))
    saveRDS(predict(mdl, dat_merged)$predictions, paste0(path_project, sprintf("AOD_predictions_spec%s_fold%s.rds", s, f)))
  }
}
print(Sys.time())
...
for (s in 1:2) {
  for (f in 1:5) {
    mdls[[m]] <- readRDS(paste0(path_project, sprintf("AOD_model_spec%s_fold%s.rds", s, f)))
    preds <- readRDS(paste0(path_project, sprintf("AOD_predictions_spec%s_fold%s.rds", s, f)))
...
saveRDS(mdls, paste0(path_project, "AOD_random_forest_models.rds"))
saveRDS(dat_preds, paste0(path_project, "AOD_random_forest_predictions.rds"))
