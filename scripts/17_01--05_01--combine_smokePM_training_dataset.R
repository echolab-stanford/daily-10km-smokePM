...
# calculate smoke PM----
smokePM <- readRDS(paste0(data_path, "/3_intermediate/station_smokePM.rds")) %>% 
...
# nlcd
nlcd <- read.csv(paste0(data_path,"/2_from_EE/NLCD_areas_10km_grid.csv")) %>% 
  filter(ID %in% epa_grid_cells) %>%    
  select(ID, groups) %>%
...
cell_cent <- st_read(paste0(data_path,"/1_grids/grid_10km_wgs84")) %>% 
  filter(ID %in% epa_grid_cells) %>%
  st_centroid %>%
  {cbind(., 
         st_coordinates(.))} %>% 
...
# AOT ----
aot_anom <- list.files(paste0(data_path, "/3_intermediate/"), 
...
fire_dist = paste0(data_path, "/distance_to_fire_cluster/") %>% 
  list.files(full.names = TRUE) %>% 
...
# AOD predictions ----
aod_pred <- list.files(paste0(output_path, "/AOD_predictions/10km_smoke_days"), 
                       full.names = TRUE,
                       pattern = "rds") %>% 
  purrr::map_dfr(function(x){readRDS(x) %>% filter(grid_id_10km %in% epa_grid_cells$grid_id_10km)})
...
# hysplit ----
hysplit <- purrr::map_dfr(list.files("~/BurkeLab Dropbox/Data/hms_hysplit/10km_grid_2006-2020",
                                     pattern = "rds",
                                     full.names = T), 
...
grid_merra = raster(list.files(paste0(data_path, "/MERRA2_AOT"), pattern = "nc",
                               full.names = TRUE))
...
pred_data %>% 
  ...
  saveRDS(paste0(data_path, "/4_clean/smokePM_training.rds"))