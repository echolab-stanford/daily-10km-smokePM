...
# Compares smoke PM2.5 at PurpleAir monitors vs 10 km grid.
#-------------------------------------------------------------------------------
...
# purpleair = readRDS(file.path(path_data, "PurpleAir", "outdoor_monitor_data_clean_part1.rds")) %>%
#   mutate(
#     # bottom code at 0
...
#   ungroup()
# 
# saveRDS(purpleair, file.path(path_data, "PurpleAir", "outdoor_monitor_data_clean_part1_daily.rds"))

purpleair = readRDS(file.path(path_data, "PurpleAir", "outdoor_monitor_data_clean_part1_daily.rds"))

#-------------------------------------------------------------------------------
#### Match PurpleAir monitors and 10 km grid cells ####
# Load PurpleAir monitor locations
purpleair_loc = readRDS(file.path(path_data, "PurpleAir", "outdoor_monitor_locs.rds")) %>% 
...
p = grid.arrange(p_comparison, p_dates, p_map,
                 layout_matrix = matrix(c(1, 1, 1, 1, 2, 2, 2, 2, 
                                          ...
                                          1, 1, 1, 1, 3, 3, 3, 3),
                                        nrow = 3, byrow = T))
ggsave(file.path(path_figures, "figureS18.png"), 
