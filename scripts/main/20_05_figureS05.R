...
# Plots supplemental figure 5.
# ------------------------------------------------------------------------------
plumes <- readRDS(file.path(path_data, "3_intermediate", "all_smoke_days.rds"))
smoke <- readRDS(file.path(path_data, "3_intermediate", "all_smoke_days_incl_cloudy.rds"))
...
epa_data %>% 
  ...
  {ggplot(data = ., aes(x = date, y = pm25)) + 
      ...
      theme_classic()} %>% 
  ggsave(file.path(path_figures, "figureS05.png"), ., 
         width = 5, height = 6)
