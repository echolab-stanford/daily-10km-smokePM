...
smoke_all <- readRDS(paste0(data_path, "/3_intermediate/all_smoke_days_incl_cloudy.rds"))  
...
map(1:12, function(month){
  ...
  month_data <- list.files(paste0(data_path, "/MERRA2_AOT"),
    ...
    left_join(smoke_all %>% select(-note_smoke_date_not_online), 
              by = c("date", "grid_id_10km")) %>% 
    replace_na(list(smoke_day = 0)) %>% 
    mutate(month = lubridate::month(date), 
           year = lubridate::year(date))
  month_medians <- nonsmoke_medians(filter(month_data, !(date %in% smoke_missing_dates)), 
