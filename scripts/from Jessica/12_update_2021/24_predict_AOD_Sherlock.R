...
path_output = paste0(path_run, "output/")
path_temp = paste0(path_run, "temp/")
if (!dir.exists(path_temp)) dir.create(path_temp)
path_preds = paste0(path_output, "AOD_predictions/")
path_preds_1km = paste0(path_preds, "1km_smoke_days/")
path_preds_10km = paste0(path_preds, "10km_smoke_days/")
if (!dir.exists(path_preds)) dir.create(path_preds)
if (!dir.exists(path_preds_1km)) dir.create(path_preds_1km)
...
#-------------------------------------------------------------------------------
# Get Anomalous AOD 1 km Predictions and 10 km Aggregations
...
xgb_mod <- xgb.load(paste0(path_output, "aod_mod.xgb"))

# Load crosswalk between 1 km grid and 10 km grid
crosswalk <- readRDS(paste0(path_data,"1_grids/grid_crosswalk_1km_10km.rds"))
...
start_time = get_start_time()
files = list.files(paste0(path_data,"2_from_EE/elevation_1km_subgrid"), 
                   full.names = TRUE)
files_filled = list.files(paste0(path_data,"2_from_EE/elevation_1km_subgrid_filled"), 
                          full.names = TRUE)
...
files = c(files_filled, paste0(path_data,"2_from_EE/elevation_1km_subgrid/", 
                               "elevation_avg_sd_1km_subgrid_", subgrids, ".csv"))
...
print(paste("NLCD", "--------------------------------------------------"))
start_time = get_start_time()
files = list.files(paste0(path_data,"2_from_EE/NLCD_1km_subgrid"), 
                   full.names = TRUE)
files_filled = list.files(paste0(path_data,"2_from_EE/NLCD_1km_subgrid_filled"), 
                          full.names = TRUE)
...
subgrids = setdiff(subgrids, subgrids_filled)
files = paste0(path_data,"2_from_EE/NLCD_1km_subgrid/", 
               "NLCD_areas_1km_subgrid_", subgrids, ".csv")
...
for (m in 1:length(year_months)) {
  ...
  # Anomalous AOT
  aot = paste0(path_data, "3_intermediate/aot_anom_smoke_days_", m_str, ".rds")
  aot = readRDS(aot) %>% filter(year(date) == as.integer(y_str))
  if (format(min(dates_m), "%Y%m%d") != start_date) {
    prev_aot = paste0(path_data, "3_intermediate/aot_anom_smoke_days_", prev_m_str, ".rds")
  ...
  out_m = foreach(d = 1:length(dates_m), .combine = c) %dopar% {
    ...
    saveRDS(new_preds, 
            paste0(path_preds_1km, "AOD_predictions_1km_", ymd_str, ".rds"))
    ...
    saveRDS(preds_10km_feats, 
            paste0(path_preds_10km, "AOD_predictions_10km_", ymd_str, ".rds"))
