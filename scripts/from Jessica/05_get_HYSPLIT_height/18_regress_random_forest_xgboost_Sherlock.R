...
path_test <- paste0(scratch, "/test_HYSPLIT/")
...
dat_panel <- readRDS(paste0(path_test, "epa_station_smokePM_full_panel_extendedCovariates.rds")) %>% 
...
# Load HYSPLIT data
dat_hysplit <- readRDS(paste0(path_test, "trajectories_72hr_CA_2020_overlap_48hr_EPA_smoke_AOT_covariates.rds")) %>% 
  ...
  for (inj_heights in ih) {
    ...
    for (cut in co) {
      ...
      for (f in 1:5) {
        ...
        if (spec == "baseline") {
          saveRDS(mdl, 
                  paste0(path_test, sprintf("HYSPLIT_model_%s_%s.rds", spec, f)))
          saveRDS(predict(mdl, dat_merged %>% select(features, -smokePM, -fold) %>% as.matrix()), 
                  paste0(path_test, sprintf("HYSPLIT_predictions_%s_%s.rds", spec, f)))
        } else {
          saveRDS(mdl, 
                  paste0(path_test, sprintf("HYSPLIT_model_%s_%s_%s_%s.rds", spec, inj, cut, f)))
          saveRDS(predict(mdl, dat_merged %>% select(features, -smokePM, -fold) %>% as.matrix()), 
                  paste0(path_test, sprintf("HYSPLIT_predictions_%s_%s_%s_%s.rds", spec, inj, cut, f)))
        }
      }
    }
  }
...
m <- 1
for (f in 1:5) {
  mdl <- readRDS(paste0(path_test, sprintf("HYSPLIT_model_baseline_%s.rds", f)))
  mdls[[m]] <- list(model = mdl,
                    spec = "baseline",
                    fold = f)
  dat_preds[, sprintf("pred.baseline.%s", f)] <- readRDS(paste0(path_test, sprintf("HYSPLIT_predictions_baseline_%s.rds", f)))
  m <- m + 1
}
for (spec in specs[-1]) {
  for (inj in gsub("\\+", "-", injection_heights)) {
    for (cut in cutoffs) {
      for (f in 1:5) {
        mdl <- readRDS(paste0(path_test, sprintf("HYSPLIT_model_%s_%s_%s_%s.rds", spec, inj, cut, f)))
        mdls[[m]] <- list(model = mdl,
                          spec = spec,
                          fold = f,
                          injection_heights = gsub("-", "\\+", inj),
                          cutoff = cut)
        dat_preds[, sprintf("pred.%s.%s.%s.%s", spec, f, cut, gsub("-", "_", inj))] <- readRDS(paste0(path_test, sprintf("HYSPLIT_predictions_%s_%s_%s_%s.rds", spec, inj, cut, f)))
        m <- m + 1
      }
    }
  }
...
saveRDS(mdls, paste0(path_test, "HYSPLIT_random_forest_models.rds"))
saveRDS(dat_preds, paste0(path_test, "HYSPLIT_random_forest_predictions.rds"))
