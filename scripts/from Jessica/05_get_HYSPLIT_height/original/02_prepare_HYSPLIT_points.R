...
# 
# Questions:
#    Are HMS HYSPLIT starting points specified in UTC? - Seems yes
#    Are HMS HYSPLIT durations in units of hours? - Seems HHMM
#    Why are there duplicate HMS HYSPLIT starting points? - Error in data recording
#    What injection heights will we use? Following Brey et al. for now
#-------------------------------------------------------------------------------
...
# Save distinct HYSPLIT points
saveRDS(dat_hms_hysplit, paste0(path_dropbox, "hms_hysplit/hms_hysplit_points_distinct.rds"))
...
o <- over(SpatialPoints(hysplit_points), us_states)
dat_hms_hysplit <- dat_hms_hysplit %>% 
  left_join(bind_cols(hysplit_points, o)) %>% 
  rename(state = NAME)
# state = NA ==> HYSPLIT points outside the US

# TO DO: MAKE COLUMN FOR COUNTRY AND GET STATES/PROVINCES OF OTHER COUNTRIES

#-------------------------------------------------------------------------------
# Save data
saveRDS(dat_hms_hysplit, paste0(path_dropbox, "hms_hysplit/hms_hysplit_initialization_distinct.rds"))
