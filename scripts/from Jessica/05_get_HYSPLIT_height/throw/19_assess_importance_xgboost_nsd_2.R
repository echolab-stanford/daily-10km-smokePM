...
dat_test <- dat_preds %>% 
  ...
  select(-test.fold, -train.cutoff, -train.injection_heights) %>% 
  left_join(dat_panel %>% select(epa_id, date, state_month, state_year, county))
...
nr <- nrow(not_smoke_day)
not_smoke_day <- not_smoke_day %>%
  mutate(injection_heights = rep(c("500+1500+2500", "500+1500", "500+2500", "1500+2500")),
         cutoff = c(10, 30, 50, 80, 100),
         spec = specs)

df <- expand.grid(injection_heights = rep(c("500+1500+2500", "500+1500", "500+2500", "1500+2500")),
            cutoff = c(10, 30, 50, 80, 100),
            spec = specs)
# df1 <- df %>% 
#   split(seq(nrow(df))) %>% 
#   lapply(bind_cols, not_smoke_day)
...
for (i in 1:nrow(df)) {
  df[i,] %>% 
    bind_cols(not_smoke_day) %>% 
    bind_rows(dat_test %>% 
                filter(injection_heights == df$injection_heights[i],
                       cutoff == df$cutoff[i],
                       spec == df$spec[i])) %>% 
    mutate(spec = factor(spec, levels = specs)) %>% 
    mutate(county = as.character(county),
...
df_imp <- importance %>% 
  ...
  ungroup() %>% 
  mutate(feature_interest = ifelse(variable %in% setdiff(specs, "baseline"), variable, "other") %>% 
...
ggplot(data = df_imp, 
  ...
  geom_pointrange(mapping = aes(xmin = importance_min, 
                                ...
                                color = feature_interest), 
                  fatten = 1.5) + 
  facet_wrap(vars(fold, spec), ncol = 4, scales = "free") + 
  scale_y_reordered() + 
