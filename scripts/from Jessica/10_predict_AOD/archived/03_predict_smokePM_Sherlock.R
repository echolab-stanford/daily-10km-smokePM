...
# 
# TODO: confirm features from AOD and HYSPLIT trajectory point counts
#-------------------------------------------------------------------------------
# Set time period
start_date = "20060101"
end_date = "20201231"
...
cell_cent = st_read(paste0(path_data, "1_grids/grid_10km_wgs84")) %>%
  st_centroid %>%
  {cbind(.,
         st_coordinates(.))} %>%
  st_drop_geometry() %>%
...
start_time = get_start_time()
elev <- paste0(path_data,"2_from_EE/elevation_avg_sd_10km_grid.csv") %>% 
  read.csv() %>% 
...
print(paste("NLCD", "--------------------------------------------------"))
start_time = get_start_time()
nlcd <- paste0(path_data,"2_from_EE/NLCD_areas_10km_grid.csv") %>% 
  read.csv() %>% 
...
for (m in 1:length(year_months)) {
  ...
  out_m = foreach(d = 1:length(dates_m), .combine = c) %dopar% {
    ...
    print(paste0(ymd_str, "smokePM predictions saved"))
    return(0)
  }
  out[[m]] = out_m
  print_time(start_time)
