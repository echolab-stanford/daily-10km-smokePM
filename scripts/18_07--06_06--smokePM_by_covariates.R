...
covar <- list.files("~/BurkeLab Dropbox/Data/US Census Bureau/American Community Survey/5-Year",
                    pattern = "2009|2019",
                    full.names = T) %>% 
...
smokePM_pred <- readRDS(paste0(output_path, "/smokePM_predictions_2006_2020.rds")) %>% 
...
g_spline_hist %>% 
  ...
  gtable_add_grob(g_maps[[4]] %>% ggplot_build() %>% ggplot_gtable(),
                  t = 1, b = 2, l = 13, r = 13) %>%
  ggsave("./figures/figure6/figure6_maps_avgs5yr.png", ., 
         width = 11, height = 4.5)
...
tract_smokePM %>% 
  ...
  ggsave("./figures/figure6/avg_individual.png", ., 
         width = 3.5*0.9, height = 5*0.9)
